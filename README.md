...
用 TypeScript 实现的终端 coding agent harness（类 Claude Code 的 MVP）：模型自主地**读代码 → 改代码 → 跑命令验证**，直到任务完成。走 Anthropic Messages API 格式（`@anthropic-ai/sdk`，流式输出），默认对接 DeepSeek 的 Anthropic 兼容端点，端点与模型完全可配。
