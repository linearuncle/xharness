...
> 首次启动会有一个风险确认——请认真读完再勾选，见下方「完全访问，但带护栏」。
...
xharness 支持两种 API 格式（供应商详情页的「API 格式」下拉可选）：**Anthropic
Messages**（`/v1/messages`）与 **OpenAI Responses**（`/v1/responses`）。Anthropic
兼容端点都能接：
...
xharness 只有一种工作模式：**完全访问**（界面上的橙色徽标）——AI 读写文件、跑命令
直接执行，不先问你。没有沙箱、没有目录限制、没有命令黑名单，效率拉满。

敢这么设计，是因为出厂内置了 **AgentGuard** 护栏插件：每条 shell 命令执行前先过一道
检查，命中常见的高危删除操作——`rm` / `unlink` / `rmdir` / `find -delete` /
`git clean`、Python 与 Node 的删除 API、以及 `DROP` / `TRUNCATE` / `DELETE FROM`
等数据库删除 SQL——就当场拦下，把决定权交还给你。别家 YOLO 模式是真裸奔，
我们大概是市面上唯一一个把"完全访问"和"防删库"同时给你的 harness。

护栏防的是粗心的 AI，不是恶意的人，也替代不了备份。这些习惯仍然建议保持：
...
AgentGuard 就是个普通插件（设置 → 插件里可启停、改规则），你也可以照着它写自己的
护栏——preToolUse hook 协议兼容 Claude Code / codex。详细威胁模型见
