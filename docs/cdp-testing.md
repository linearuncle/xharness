...
2. 执行改动所需的类型检查、单元测试等代码级验证。
3. 在仓库根目录执行 `npm run build`，保证 GUI 使用最新的 `dist/`。
4. 完全重启 GUI 开发实例，不能复用实现前启动的旧进程。
5. 执行第 3 节的三段标准冒烟检查。
6. 按第 5 节设计并执行本次需求的专项检查。
7. 任何检查失败时，修复后从构建、重启、标准冒烟开始重新执行。
8. 按第 6 节报告验证证据；不能只写“已测试”或“看起来正常”。
